...
public abstract class RosAppActivity extends RosActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (mainWindowId == 0) {
            Log.e("ApplicationManagement", "You must set the dashboard resource ID in your RosAppActivity");
            return;
        }
    ...
    protected void init(NodeMainExecutor nodeMainExecutor) {
        ...
        if (managedApplication) {
            if (getIntent().hasExtra(RobotDescription.UNIQUE_KEY)) {
                robotDescription = (RobotDescription) getIntent().getSerializableExtra(RobotDescription.UNIQUE_KEY);
            }
            nodeMainExecutor.execute(managedPairingApplicationNamePublisher, nodeConfiguration.setNodeName("pairingApplicationNamePublisher"));
        }
        if (robotDescription != null) {
            robotNameResolver.setRobot(robotDescription);
            dashboard.setRobotName(robotDescription.getRobotType());
        }
        nodeMainExecutor.execute(robotNameResolver, nodeConfiguration.setNodeName("robotNameResolver"));
...
//				restartApp();
    ...
    private void startApp() {
        ...
        AppManager appManager = new AppManager(robotAppName, getRobotNameSpace());
